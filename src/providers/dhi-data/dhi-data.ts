...
export class DhiDataProvider {
  ...
  getAllShelvesInSection(whseID: number, aisle: number, section: number) {
    return this.http.get(
      this.url +
        `getAllShelvesInSection?whseID=${whseID}&aisle=${aisle}&section=${section}`
    );
  }
  ...
  getAllItemsInSection(whseID: number, aisle: number, section: number) {
    ...
    return this.http.get(
      this.url +
        `GetAllItemsInSection?whseID=${whseID}&aisle=${aisle}&section=${section}`
    );
  }

  getItemLocations(whseID: number, itemID: number) {
    //(int whseID, int aisle, int section)
    console.log(itemID, whseID);
    return this.http.get(
      this.url + `GetItemLocations?itemID=${itemID}&whseID=${whseID}`
    );
  }

  getLocationsByCustomer(custID: number, whseID: number) {
    console.log(custID, whseID);
    return this.http.get(
      this.url + `GetLocationsByCustomer?custID=${custID}&whseID=${whseID}`
    );
  }
  ...
  getWarehouses() {
    if (this._warehouses) {
      return Observable.of(this._warehouses);
    } else if (this.observable) {
      return this.observable;
    } else {
      this.observable = this.http.get(this.url + "GetWarehouses").map(resp => {
